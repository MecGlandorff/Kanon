...
export function analyzeRepo(root = process.cwd(), options = {}) {
  ...
  const scanned = scanRepo(resolvedRoot, options.scan);
  const evidence = new EvidenceBook(options.runId);
  const files = scanned.files;

  const readmeFile = findFirst(files, [
    "README.md",
    "README.mdx",
    "README.txt",
    "README"
  ]);
  const readmeText = readmeFile ? readText(resolvedRoot, readmeFile.path) : "";
...
function detectTests(files, evidence, packageInfo, pyprojectInfo, pythonInfo) {
  ...
  const hasPytest =
    ...
    testFiles.some((file) => file.extension === ".py");
...
function detectPurpose(readmeFile, readmeText, readmeEvidence, packageInfo, pyprojectInfo, evidence) {
...
function buildCurrentState(input) {
  ...
  if (input.purpose.confidence !== "unknown") {
    known.push({
