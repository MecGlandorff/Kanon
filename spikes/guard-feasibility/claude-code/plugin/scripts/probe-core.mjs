...
const COVERED_TOOLS = new Set(["Bash", "Write", "Edit"]);
...
export function inspectHook(input, environment = process.env) {
  ...
  const pluginData = probePluginData(environment.CLAUDE_PLUGIN_DATA);
  const observation = {
    ...
    plugin_root_present: Boolean(
      boundedString(environment.CLAUDE_PLUGIN_ROOT, MAX_ID_LENGTH)
    ),
