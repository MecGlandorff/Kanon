...
export function buildRefactorPlan(analysis, options = {}) {
  ...
  const hotspots = rankHotspots([
    ...
    ...duplicationFamilies,
    ...(scanned.diagnostics.complete ? deadCodeHotspots(fileMetrics, analysis.state) : [])
  ]);
...
function buildOneSessionPlan(state, primary, secondary, answers, scan) {
  ...
  if (!scan.complete) {
    steps.push("Treat detected hotspots as partial because the repository scan was incomplete; do not infer dead code from missing references.");
  }
  if (primary) {
